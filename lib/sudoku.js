...
class SudokuBoard {
  ...
  checkForErrors() {
    this.clearErrors();
    const cells = {};
    const rows = this.checkRowsForErrors(cells);
    const cols = this.checkColsForErrors(cells);
    const boxes = [];
    let error;
  ...
  checkRowsForErrors(cells) {
    let idx;
    let rows = [];
    // debugger
    for (let i = 0, len = Math.sqrt(this.cells.length); i < len; i++) {
      // debugger
      let thisRow = [false,false,false,false,false,false,false,false,false];
      for (let j = 0; j < len; j++) {
        idx = (i * 9) + j;
        if (this.cells[idx].currentValue === 0) {
          continue;
        } else if (thisRow[this.cells[idx].currentValue - 1] !== false) {
          cells[idx] = true;
          cells[thisRow[this.cells[idx].currentValue - 1]] = true;
          rows.push(i);
        } else {
          thisRow[this.cells[idx].currentValue - 1] = idx;
        }
      }
    }
    return rows;
  ...
  checkColsForErrors(cells) {
    let idx;
    let cols = [];
    for (let i = 0, len = Math.sqrt(this.cells.length); i < len; i++) {
      let thisCol = [false,false,false,false,false,false,false,false,false];
      for (let j = 0; j < len; j++) {
        idx = (j * 9) + i;
        if (this.cells[idx].currentValue === 0) {
          continue;
        } else if (thisCol[this.cells[idx].currentValue - 1] !== false) {
          cells[idx] = true;
          cells[thisCol[this.cells[idx].currentValue - 1]] = true;
          cols.push(i);
        } else {
          thisCol[this.cells[idx].currentValue - 1] = idx;
        }
      }
    }
    return cols;
